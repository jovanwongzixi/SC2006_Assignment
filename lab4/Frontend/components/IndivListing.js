...
export default function IndivListing({ displayValues, hdbType }) {
    const router = useRouter()

    function computeFlatTypes() {
        let flatTypes = ''
        flatTypesArr.forEach((val) => {
            if (displayValues[val] > 0) flatTypes += `${capitalise(val)},`
        })
    ...
    const handleListingClick = () => {
        ...
        console.log(
            JSON.parse(decodeURIComponent(encodeURIComponent(JSON.stringify(displayValues))))
        )
        router.push({
            pathname: `/${hdbType}`,
            query: {
