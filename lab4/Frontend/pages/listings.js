...
const fetcher = (...args) =>
    fetch(...args).then((res) => {
        console.log('usingSWR')
        return res.json()
    })

function ListingDiv({ queryString, page, hdb_type }) {
    const apiUrl =
        `https://goodstart-backend.fly.dev/api/flat_data/${hdb_type}/query?page=${page - 1}` +
        queryString
        ...
        <>
            {apiData !== undefined &&
                apiData.map((value, index) => (
                    <IndivListing key={index} displayValues={value} hdbType={hdb_type} />
                ))}
        </>
    )
}
...
export default function Listings({ towns, amenities, flat_types, price_range }) {
    ...
    let queryString = ''
    let hdbType = 'resale' // default to resale if undefined
    for (const [key, value] of Object.entries(routerData)) {
        if (key === 'town' && value !== '') {
            queryString += `${value !== undefined ? '&' + key + '=' + value : 'bishan'}`
        } else if (key === 'hdb_type' && value !== '') {
            hdbType = value.toLowerCase()
    ...
    const { data: queryResultSize, isLoading } = useSWR(
        `https://goodstart-backend.fly.dev/api/flat_data/${hdbType}/query_size?` + queryString,
        fetcher
    )
    console.log(isLoading)
    const paginationCount =
        queryResultSize === undefined
            ? 0
            : Math.ceil(queryResultSize.query_size / queryResultSize.page_size)
    return (
        <>
            <div className={style.maindiv}>
                ...
                <div className={style.listingsdiv}>
                    <ListingDiv queryString={queryString} page={page} hdb_type={hdbType} />
                    <Pagination count={paginationCount} page={page} onChange={handlePageChange} />
                </div>
            </div>
            {/* preload next page into cache for better UX */}
            <div style={{ display: 'none' }}>
                <ListingDiv queryString={queryString} page={page + 1} hdb_type={hdbType} />
            </div>
        </>
    )
}
