import { Fade, TextField, Autocomplete, Box, Slider, Button, FormControl, FormControlLabel, FormLabel, Radio, RadioGroup, InputLabel, Select, OutlinedInput, MenuItem, Menu, Popper, Typography, IconButton, Collapse} from "@mui/material"
...
export default function Searchbox({ towns, amenities, flat_types, price_range }){
    ...
    const handleOnClickAway = (event) => {
        ...
        if(event.target !== document.querySelector("#advancedOptions") && event.target !== document.body){
            setOpen(false)
            if(event.target === document.querySelector(`div input[name='town']`)){console.log(event.target); setSearchboxFocused('town')}
    ...
    const handleOnFocus = (name) => {
        ...
        console.log('focusing');setSearchboxFocused(prevVal => {
    ...
    const handleSearch = (event) => {
        ...
        router.push({
            ...
            query: {
                town: inputTownValue,
                min_price: value.priceRange[0],
                max_price: value.priceRange[1],
                amenities: value.nearbyAmenities.toString().toLowerCase(),
                flat_types: value.flatTypes.toString().toLowerCase(),
                hdb_type: value.HDBType
            }
        })
    ...
    return(
        ...
        <div>
            <div id="searchboxid" className={style.searchbox} style={{backgroundColor: searchboxFocused === '' ? '#fff': '#E3E3E3'}}>
                <CustomAutocompleteTowns
                    ...
                    renderInput={(params) => {
                        ...
                        return (<div style={{position: 'relative'}} ref={params.InputProps.ref}>
                            <label style={{position: 'absolute', color:'black', left:18, top:5, fontWeight:500, fontSize: '0.8rem'}}>Town</label>
                            <input 
                                autoFocus={searchboxFocused==='town'}
                                name='town' onBlurCapture={handleOnBlur}
                ...
                <UnstyledSelectSimple
                    ...
                    value={value.HDBType}
                    onChange={(e,newValue)=>setValue(prevValue=>{return{...prevValue, HDBType: newValue}})}
